...
function PlusPack({com, setActivemodal}){
    ...
    const createPack = async () => {
        if(name.length > 2 && price.length> 2){
            try{
                const {data} = await NewsService.createPack({capter: com, name, time, desc: desclist, price, priory, image: imgpage})
                if(data){
    ...
    return(
        <div className={style.main}>
            <div className={style.left_path}>
                ...
                <div className={style.price}>12000</div>
            ...
            <div className={style.line_btn} onClick={createPack}>Создать</div>
        </div>
    )
}

export default PlusPack