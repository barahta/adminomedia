...
function PlusPackReaFarm({setActivemodal}) {
    ...
    const createPost = async () => {
        if(namepost.length>0 && descpost !== ''){
            try{
                const {data} = await NewsService.createPack({capter: company, name: namepost, time: descpost, desc: view, price: '', image: imgpage, publicdesc: '', priory: 0})
                if(data){
...
export default PlusPackReaFarm;